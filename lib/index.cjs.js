...
var DraftOffsetKey = _interopDefault(require('draft-js/lib/DraftOffsetKey'));
  ...
  return function (WrappedComponent) {
    ...
    function (_Component) {
      ...
      function BlockFocusDecorator() {
        ...
        _this = _possibleConstructorReturn(this, (_getPrototypeOf2 = _getPrototypeOf(BlockFocusDecorator)).call.apply(_getPrototypeOf2, [this].concat(args)));

        _defineProperty(_assertThisInitialized(_this), "onClick", function (evt) {
          evt.preventDefault();
    ...
    }(React.Component), _defineProperty(_class, "displayName", "BlockFocus(".concat(getDisplayName(WrappedComponent), ")")), _defineProperty(_class, "WrappedComponent", WrappedComponent.WrappedComponent || WrappedComponent), _temp;
  };
});
...
function removeBlock (editorState, blockKey) {
  ...
  if (beforeBlock === undefined) {
    ...
    var _newState = draftJs.EditorState.push(editorState, content, 'remove-block'); // force to new selection
  ...
  var newState = draftJs.EditorState.push(editorState, content, 'remove-block'); // force to new selection
    ...
    onChange: function onChange(editorState) {
      ...
      if (lastSelection) {
        ...
        if (lastBlockMapKeys.some(function (key) {
          ...
          return draftJs.EditorState.forceSelection(editorState, editorState.getSelection());
        }
      }
